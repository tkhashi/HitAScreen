...
- [ ] P1-6: ラベル外観設定（色/透明度/サイズ/フォントサイズ）を追加
  - 作業内容:
...
- [ ] P4-2: 初回ホットキー時のラベル縦ズレ修正

- [ ] P4-3: 2回目以降ヘルプテキスト隠れ修正
...
- [ ] P5-1: Release workflow 作成（タグ `vX.Y.Z` 起動）
  - 作業内容:
...
- [ ] P5-5: codesign + entitlements 組み込み
  - 作業内容:
    - `.app` と内部実行ファイルを署名
...
- [ ] P5-8: GitHub Release へ資産アップロード
  - 作業内容:
    - `HitAScreen-<version>-macos-arm64.dmg` と `SHA256SUMS.txt` を添付
  - 完了条件:
    - Release 画面で資産が確認できる

- [ ] P5-9: README に導入手順/権限付与手順を追記
  - 作業内容:
    - Release ダウンロード手順
    - 初回起動時の権限設定手順
  - 完了条件:
